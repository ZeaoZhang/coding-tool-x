...
## [Unreleased]
### Added
- **配置驱动的平台 Registry** - 新增 Manifest 校验、allowlisted capability driver、`~/.cc-tool/config/platforms.json` 用户扩展和安全的 `GET /api/platforms` 公共目录；`generic-jsonl`、`generic-filesystem` 与 `generic-openai-compatible` 仅覆盖可声明的稳定协议。
- **Registry-backed CLI / Web UI 路由** - 代理、日志、统计、端口配置、Dashboard 聚合和前端平台元数据改为从 Registry 派生，同时保留旧 CLI/API facade、OMP managed mode 与现有测试 mock 路径。
...
### Changed
- **兼容性边界明确** - 用户配置禁止任意 Node.js 模块、函数值和 shell 命令；特殊 SQLite、OAuth/keychain、非标准配置及 fork 语义继续由专用 capability driver 负责。
- **手动刷新边界** - Skill 页面进入、浏览器刷新、切换页面和添加/修改仓库只读取或保存本地状态；只有用户手动点击刷新才触发远端网络任务

### Security
- **控制面安全** - MCP/Skill 控制清单不保存完整 secret；项目 MCP 静态 secret 默认拒绝；LAN 远程写默认关闭，并要求 same-origin/loopback 控制
...
### Added
- **OMP 技能扫描设置** - OMP 技能独立页与抽屉新增四项来源开关，专用 API 白名单更新 `config.yml` 时保留其他配置，保存后自动刷新技能列表

### Fixed
- **动态切换渠道 Key 展示修复** - WebSocket 代理状态广播中的脱敏渠道信息不再覆盖前端已加载的完整渠道对象，避免动态切换后渠道面板将已配置的 API Key 误显示为未设置
- **渠道 Key 脱敏态文案修复** - 渠道面板现在区分“后端未返回敏感字段”和“确实未配置 Key”，脱敏字段显示为“已隐藏”
- **Gemini / OpenAI 兼容代理路径去重** - 代理目标 URL 现在会同时去重重复的 `/v1` 和 `/v1beta` 版本段，避免 baseUrl 与请求路径都包含版本前缀时拼出重复路径
...
- **会话详情轻量目录接口** - 新增会话 `outline` 与 `status` 轻量接口，为详情抽屉左侧用户目录和实时同步提供独立数据源

### Fixed
- **会话详情加载过慢** - 聊天详情抽屉左侧“用户目录”不再依赖前端预加载整段历史，而是改为轻量 outline 加载，显著减少大会话打开时的等待时间
- **会话历史非实时刷新** - 打开详情后会基于轻量状态轮询自动增量刷新当前会话，不再需要用户每次重新打开会话才能触发同步
- **Windows Fork 文件换行兼容** - Fork 截断会保留原始会话文件换行风格，避免在 Windows 上将 `CRLF` 会话意外改写为 `LF`
