'use strict';
describe('claude Proxy Driver', () => { test('exposes structured lifecycle operations', async () => { const { createDriver } = require('../../../../src/platforms/drivers/claude/proxy'); const driver = createDriver({ requireImpl: () => ({ getProxyStatus: () => ({ running: false }), startProxyServer: options => ({ running: true, options }), stopProxyServer: options => ({ running: false, options }) }) }); expect(driver.status()).toMatchObject({ status: 'ok', platform: 'claude', capability: 'proxy', operation: 'status', data: { running: false } }); await expect(driver.start({ port: 1234 })).toMatchObject({ status: 'ok', operation: 'start', data: { running: true, options: { port: 1234 } } }); }); });

const http = require('http');
const CLAUDE_PROXY_PATH = require.resolve('../../../../src/platforms/drivers/claude/proxy-implementation');
const HTTP_PROXY_PATH = require.resolve('http-proxy');
const CONFIG_LOADER_PATH = require.resolve('../../../../src/config/loader');
const CLAUDE_CHANNELS_PATH = require.resolve('../../../../src/platforms/drivers/claude/channels-implementation');
...
const EVENT_BUS_PATH = require.resolve('../../../../src/plugins/event-bus');
const TOKEN_RECOVERY_PATH = require.resolve('../../../../src/platforms/drivers/claude/token-recovery');
const OPENAI_GATEWAY_PATH = require.resolve('../../../../src/platforms/drivers/claude/openai-gateway');
...
function request(port) {
  return new Promise((resolve, reject) => {
    ...
    req.on('error', reject);
    req.end(JSON.stringify({ model: 'claude-3-5-sonnet' }));
  });
}
...
beforeEach(async () => {
  ...
  stub(CLAUDE_CHANNELS_PATH, {
    getEffectiveApiKey: vi.fn(() => 'upstream-key'),
    getClaudeProxyExcludedChannelIds: vi.fn(() => excludedChannelIds)
  });
  ...
  stub(PROXY_UTILS_PATH, {
    ...
    normalizeGatewaySourceType: (value, fallback) => value || fallback
  });
  ...
  stub(EVENT_BUS_PATH, { emitSync: vi.fn() });
  stub(TOKEN_RECOVERY_PATH, {
    createClaudeStreamRecoveryState: vi.fn(() => ({})),
    mergeClaudeStreamEvent: vi.fn(),
    buildAssistantMessageFromStreamState: vi.fn(() => null),
    recoverClaudeUsageViaCountTokens: vi.fn(async () => null)
  });
  stub(OPENAI_GATEWAY_PATH, {
    handleClaudeOpenAiGatewayRequest: vi.fn(async () => false)
  });
  ...
  delete require.cache[CLAUDE_PROXY_PATH];
  claudeProxy = require('../../../../src/platforms/drivers/claude/proxy-implementation');
});

afterEach(async () => {
  await claudeProxy?.stopProxyServer?.();
  delete require.cache[CLAUDE_PROXY_PATH];
...
test('blocks Claude proxy startup while an OAuth channel is enabled', async () => {
  excludedChannelIds = ['oauth-channel'];

  await expect(claudeProxy.startProxyServer()).rejects.toThrow(
    'Claude dynamic proxy supports API-key channels only'
  );
});

test('excludes OAuth channels and injects the allocated API key into requests', async () => {
  const started = await claudeProxy.startProxyServer();
  excludedChannelIds = ['oauth-channel'];

  const response = await request(started.port);

  expect(response.statusCode).toBe(200);
  expect(allocationOptions).toMatchObject({
    source: 'claude',
    enableSessionBinding: true,
    excludeChannelIds: ['oauth-channel']
  });
  expect(capturedRequest.selectedChannel).toBe(claudeChannel);
