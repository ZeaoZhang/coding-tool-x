...
export const MINIMAL_PLATFORM_FALLBACK = [
  ...
  { key: 'dsh', label: 'DSH', title: 'DeepSeek Harness', command: 'dsh', iconToken: 'terminal', color: '#2563eb', defaultVisible: true, capabilities: { channels: true, projects: true, sessions: true, proxy: true, statistics: true, nativeLogs: true, nativeConfig: true, api: true, mcp: true, prompts: true }, resourceTypes: { skills: true, commands: false, agents: false, plugins: true, mcp: true, prompts: true }, resourceActions: { skills: { create: false, repositories: true, import: false, refresh: true }, plugins: { repositories: true, market: false, install: false, uninstall: false, import: false, syncRepos: false } } }
]
...
export function normalizePublicPlatform(platform = {}) {
  ...
  return {
    ...
    defaultEnabled: platform?.defaultEnabled === undefined ? undefined : platform.defaultEnabled !== false,
    sortOrder: Number.isFinite(platform?.sortOrder) ? platform.sortOrder : 0,
    portKey: typeof platform?.portKey === 'string' ? platform.portKey : '',
    defaultPort: Number.isFinite(platform?.defaultPort) ? platform.defaultPort : null,
