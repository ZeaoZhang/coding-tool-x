...
function clearChannelBalanceCache(channel) {
  try {
    require('../../../server/services/channel-balance').clearChannelBalanceCache('gemini', channel);
  } catch (_) {
...
function invalidateDashboardSource() {
  try {
    require('../../../server/services/snapshot-cache').invalidateDashboardSourceSnapshot('gemini');
  } catch (_) {}
}
