...
function resolveTarget(manifest) {
  ...
  if (home && !isInside(home, target)) throw new Error('Prompt mapping escapes platform home');
  return target;
}
...
async function canonicalPath(fsImpl, target) {
  ...
  while (true) {
    try {
      const canonical = await fsImpl.realpath(current);
...
async function assertSafePath(fsImpl, target, home, allowMissing = false) {
  ...
  const checkComponents = async value => {
    ...
    for (const part of parts) {
      ...
      try {
        ...
        if (stat.isSymbolicLink()) {
          ...
          if (!isSystemAlias(current, real)) throw new Error(`Prompt path contains symlink: ${current}`);
        }
      } catch (error) {
        if (error.code !== 'ENOENT') throw error;
        missing = true;
    ...
    if (missing && !allowMissing) {
      const error = new Error(`Prompt path does not exist: ${resolved}`);
      error.code = 'ENOENT';
      throw error;
    }
  };
  ...
  if (!isInside(canonicalHome, canonicalTarget)) throw new Error('Prompt mapping escapes platform home');
}
...
async function createCanonicalTarget(fsImpl, target, home, allowMissing) {
  ...
  if (canonicalHome && (!isInside(canonicalHome, canonicalParent) || !isInside(canonicalHome, canonicalTarget))) {
    throw new Error('Prompt mapping escapes platform home');
  }
...
async function revalidateCanonicalTarget(fsImpl, canonical, target, home) {
  ...
  if (currentParent !== canonical.parent || currentTarget !== canonical.target || currentHome !== canonical.home) {
    throw new Error('Prompt target changed during operation');
  }
...
async function readFileByDescriptor(fsImpl, target, home) {
  ...
  if (typeof fsImpl.open !== 'function') throw new Error('Prompt descriptor read is unavailable');
  ...
  try {
    ...
    if (!descriptorStat) throw new Error('Prompt descriptor identity is unavailable');
    ...
    if (typeof pathStat.isSymbolicLink === 'function' && pathStat.isSymbolicLink()) {
      throw new Error(`Prompt path contains symlink: ${canonical.target}`);
    }
    if (!sameFileIdentity(descriptorStat, pathStat)) throw new Error('Prompt target changed during operation');
...
async function ensureDirectory(fsImpl, directory, canonicalHome) {
  ...
  const assertDirectory = async (component, stat) => {
    if (typeof stat.isSymbolicLink === 'function' && stat.isSymbolicLink()) {
      ...
      if (!isSystemAlias(component, real)) throw new Error(`Prompt path component is not a directory: ${component}`);
    } else if (typeof stat.isDirectory === 'function' && !stat.isDirectory()) {
      throw new Error(`Prompt path component is not a directory: ${component}`);
    }
  };
...
async function atomicWrite(fsImpl, target, content, canonicalHome) {
  ...
  const temporary = path.join(parent, tempName);
  let handle;
  try {
    const canonicalParent = await canonicalPath(fsImpl, parent);
    if (canonicalHome && !isInside(canonicalHome, canonicalParent)) throw new Error('Prompt mapping escapes platform home');
    ...
    const canonicalTemporary = path.join(canonicalParent, tempName);
    if (typeof fsImpl.open === 'function') {
      handle = await fsImpl.open(canonicalTemporary, 'wx', 0o600);
      await handle.writeFile(content, 'utf8');
      await handle.close();
      handle = undefined;
    } else {
      await fsImpl.writeFile(canonicalTemporary, content, { encoding: 'utf8', flag: 'wx', mode: 0o600 });
    }
    await assertSafePath(fsImpl, canonicalTemporary, canonicalHome, false);
    if (await canonicalPath(fsImpl, canonicalTemporary) !== canonicalTemporary) throw new Error('Prompt temporary path changed during write');
    ...
    await fsImpl.rename(canonicalTemporary, target);
  } finally {
    if (handle) { try { await handle.close(); } catch { /* best effort close */ } }
    try {
      if (typeof fsImpl.rm === 'function') await fsImpl.rm(temporary, { force: true });
      else if (typeof fsImpl.unlink === 'function') await fsImpl.unlink(temporary);
...
function createGenericPromptDriver({ platform, manifest = {}, fsImpl = fs } = {}) {
  const read = async () => {
    try {
      ...
      return await readFileByDescriptor(fsImpl, target, manifest.paths && manifest.paths.home);
  ...
  const write = async text => {
    try {
      ...
      await atomicWrite(fsImpl, canonical.target, text, canonical.home);
      return { status: 'ok', platform, capability: 'prompts', operation: 'write', target };
  ...
  const remove = async () => {
    try {
      ...
      return { status: 'ok', platform, capability: 'prompts', operation: 'remove', target };
    } catch (error) { return failure({ platform, operation: 'remove', error }); }
  };
  return { platform, capability: 'prompts', read, write, remove };
}
