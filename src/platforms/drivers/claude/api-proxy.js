...
const { startProxyServer, stopProxyServer, getProxyStatus } = require('./proxy-implementation');
const {
  setProxyConfig,
  restoreSettings,
  deleteBackup,
...
const { getAllChannels, markChannelAsRecentlyUsed, extractApiKeyFromHelper } = require('./channels-implementation');
const { clearNativeOAuth } = require('../../native-oauth-adapters');
const { clearAllLogs } = require('../../../server/websocket-server');
...
function findActiveChannelFromSettings() {
  try {
    ...
    const { readNativeOAuth } = require('../../native-oauth-adapters');
...
router.post('/start', async (req, res) => {
  try {
    ...
    const { broadcastProxyState } = require('../../../server/websocket-server');
...
router.post('/stop', async (req, res) => {
  try {
    ...
    if (restoredChannel) {
      ...
      const channels = getAllChannels();
      const currentSettings = require('./channels-implementation').getCurrentSettings();
      if (currentSettings) {
    ...
    if (restoredChannel) {
      const { applyChannelToSettings } = require('./channels-implementation');
