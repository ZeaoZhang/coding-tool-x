...
const USAGE_SOURCE = 'claude';

function toLegacyEntryShape(entry = {}, includeName = false) {
  const normalized = normalizeUsageTokens(USAGE_SOURCE, entry.tokens || {});
  const result = {
...
function recordRequest(requestData = {}) {
  const normalizedTokens = normalizeUsageTokens(USAGE_SOURCE, requestData.tokens || {});
