...
Coding-Tool-X is a Node-based CLI whose public entry point is `bin/ctx.js`, and `src/index.js` orchestrates menu routing plus command handlers. CLI actions live under `src/commands`, shared helpers sit in `src/utils`, and configuration schemas stay in `src/config`. The Express proxy/websocket backend resides in `src/server` (api, services, proxy-server). The Vue 3 dashboard lives under `src/web` with composables/components/views; install and script inside that folder separately. Docs and marketing assets stay in `docs/`. User-level data lives at `~/.claude/ctx/` and must remain untracked.

## Build, Test, and Development Commands
- `npm start` - runs the CLI exactly as `ctx`.
...
- `ctx ui`, `ctx proxy start|stop|status` - manual verification commands; run them before shipping.
...
## Testing Guidelines
`npm test` is currently a placeholder, so accompany critical changes with targeted scripts or harnesses. Add integration tests for command handlers (e.g., spawn `bin/ctx.js` with fixtures) and component tests beside Vue files named `Component.spec.ts`. Manual smoke tests should cover `ctx ui`, proxy lifecycle, and multi-channel switching; capture console output in PRs until automated coverage exists.
...
Secrets and API tokens belong in `~/.claude/ctx/channels.json`; never inline them in commits. When adding channels or proxy options, thread the config through `src/config` loaders and document defaults in `README.md`. Validate user paths and HTTP targets inside `src/server/services` to avoid shell injection, and run `publish.sh` only after confirming no private data lands in `dist/` or npm artifacts.
